...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde'])) && !empty(isset($_REQUEST['vdesde'])) ) {
            if ($_REQUEST['opcion']==1) {
                $cargo = consultas::get_datos("select * from cargo where car_cod between " .$_REQUEST['vdesde']. " and " .$_REQUEST['vhasta'] ); 
                
            }else {
                
            $cargo = consultas::get_datos("select * from cargo where car_descri between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."' " );
               } 
            }else{
