...
// AGREGAR PAGINA
$pdf->AddPage('P', 'LEGAL');
$pdf->Cell(0, 0, "ORDEN DE PRODUCCION", 0, 1, 'C');
//SALTO DE LINEA
$pdf->Ln();
if (!empty(isset($_REQUEST['opcion']))) {
    switch ($_REQUEST['opcion']) {
        case 1://fecha            
            $cabeceras = consultas::get_datos("select * from v_ord_pro "
                            . "where ped_fecha::date between '" . $_REQUEST['vdesde'] . "' and '" . $_REQUEST['vhasta'] . "'");
            break;
        case 2: //cliente
            $cabeceras = consultas::get_datos("select * from v_ord_pro where prv_cod in(" . $_REQUEST['vproveedor'] . ")");
            break;
        case 3: //articulo
            $cabeceras = consultas::get_datos("select * from v_ord_pro "
                            . "where ordpro_cod in(select ordpro_cod from detalle_pedventa where art_cod in(" . $_REQUEST['varticulo'] . "))");
            break;
        case 4: //empleado
            $cabeceras = consultas::get_datos("select * from v_ord_pro where emp_cod in(" . $_REQUEST['vempleado'] . ")");
            break;
    }
} else {
    $cabeceras = consultas::get_datos("select * from v_ord_pro where ordpro_cod =" . $_REQUEST['vordpro_cod']);
}
$pdf->SetFont('times', '', 11);
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        $pdf->Cell(120, 2, "CLIENTE: " . $cabecera['clientes'], 0, '', 'L');
        $pdf->Cell(60, 2, "PEDIDO N°: " . $cabecera['nro_orden'], 0, '', 'L');
        
        $pdf->Ln();
        $pdf->Cell(120, 2, "ELABORADO POR: " . $cabecera['empleado'], 0, '', 'L');
        $pdf->Cell(60, 2, "FECHA: " . $cabecera['orden_fecha'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(120, 2, "SUCURSAL: " . $cabecera['suc_descri'], 0, '', 'L');
        $pdf->Cell(60, 2, "ESTADO: " . $cabecera['orden_estad'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(0, 5, "", 0, '', 'L');
        $pdf->Ln();
//COLOR DE TABLA
        ...
        $detalles = consultas::get_datos("select * from v_det_orden where ordpro_cod=" . $cabecera['ordpro_cod']);
        if (!empty($detalles)) {
