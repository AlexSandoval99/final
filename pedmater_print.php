...
if (!empty(isset($_REQUEST['opcion']))) {
    switch ($_REQUEST['opcion']) {
        case 1://fecha            
            ...
            break;
        case 2: //cliente
            $cabeceras = consultas::get_datos("select * from v_pedido_cabcompra where prv_cod in(" . $_REQUEST['vproveedor'] . ")");
            break;
        case 3: //articulo
            $cabeceras = consultas::get_datos("select * from v_pedido_cabcompra "
                            . "where ped_com in(select ped_com from detalle_pedventa where art_cod in(" . $_REQUEST['varticulo'] . "))");
            break;
        case 4: //empleado
            $cabeceras = consultas::get_datos("select * from v_pedido_cabcompra where emp_cod in(" . $_REQUEST['vempleado'] . ")");
            break;
    }
} else {
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(60, 2, "PEDIDO N°: " . $cabecera['ped_com'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(0, 5, "", 0, '', 'L');
        $pdf->Ln();
//COLOR DE TABLA
...
//SALIDA AL NAVEGADOR
$pdf->Output('reporte_pedidocompra.pdf', 'I');
?>