...
if (!empty(isset($_REQUEST['opcion']))) {
    switch ($_REQUEST['opcion']) {
        case 1://fecha            
            $cabeceras = consultas::get_datos("select * from v_pedido_cabventa "
                            . "where ped_fecha::date between '" . $_REQUEST['vdesde'] . "' and '" . $_REQUEST['vhasta'] . "'");
            break;
        case 2: //cliente
            $cabeceras = consultas::get_datos("select * from v_pedido_cabventa where cli_cod in (". $_REQUEST['vcliente'].")");
            break;
        case 3: //articulo
            $cabeceras = consultas::get_datos("select * from v_pedido_cabventa "
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(80, 2, "ESTADO: " . $cabecera['estado'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(130, 2, "SUCURSAL: " . $cabecera['suc_descri'], 0, '', 'L');
        $pdf->Cell(80, 2, "PEDIDO N°: " . $cabecera['ped_cod'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(0, 5, "", 0, '', 'L');
        $pdf->Ln();
//COLOR DE TABLA
