...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde'])) && !empty(isset($_REQUEST['vdesde'])) ) {
            if ($_REQUEST['opcion']==1) {
                $sucur = consultas::get_datos("select * from sucursal where id_sucursal between " .$_REQUEST['vdesde']. " and " .$_REQUEST['vhasta'] ); 
                
            }else {
                
            $sucur = consultas::get_datos("select * from sucursal where suc_descri between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."' ORDER BY suc_descri " );
               } 
            }else{
                
                 $sucur = consultas::get_datos("select * from sucursal ");
            }
            
            foreach ($sucur as $sucur) {
            $pdf->Cell(50,5,$sucur['id_sucursal'], 1, 0, 'C', 1);
            $pdf->Cell(0,5,$sucur['suc_descri'], 1, 0, 'L', 1);
            $pdf->Ln();
        }
