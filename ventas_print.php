...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(80, 2, "FECHA: " . $cabecera['ven_fecha'], 0, '', 'L');
        $pdf->Ln();   
        $pdf->Cell(130, 2, "ELABORADO POR: " . $cabecera['empleado'], 0, '', 'L');
        $pdf->Cell(80, 2, "ESTADO: " . $cabecera['ven_estado'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(130, 2, "SUCURSAL: " . $cabecera['suc_descri'], 0, '', 'L');
        $pdf->Cell(80, 2, "PEDIDO N°: " . $cabecera['ven_cod'], 0, '', 'L');
        $pdf->Ln();
        $pdf->Cell(0, 5, "", 0, '', 'L');
        $pdf->Ln();
//COLOR DE TABLA
        ...
        if (!empty($detalles)) {
            ...
            $pdf->Cell(60, 2, number_format($cabecera['ven_total'], 0, ",", "."), 1, 0, 'R', 1);
            $pdf->Ln();
            $pdf->Ln();
            $pdf->SetFont('times', '', 11);
        } else {
